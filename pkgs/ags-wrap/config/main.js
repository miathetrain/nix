...
notifications.popupTimeout = 6e3;
var popups = notifications.bind("popups");
...
var notificationPopup = Widget.Window({
  name: "notifications",
  anchor: ["top", "right"],
  layer: "overlay",
  child: Widget.Box({
    ...
    css: "padding: 1px;",
    vertical: true,
    children: popups.as((popups2) => popups2.map(Notification))
  })
});
...
var scss = `${App.configDir}/css/style.scss`;
var css = `/tmp/my-style.css`;
...
Utils.monitorFile(
  ...
  // reload function
  function() {
    const scss2 = `${App.configDir}/css/style.scss`;
    const css2 = `/tmp/my-style.css`;
    Utils.exec(`sassc ${scss2} ${css2}`);
    App.resetCss();
    App.applyCss(css2);
  }
);
var Launcher = Widget.Icon({
  class_name: "launcher",
  icon: "/home/mia/.config/ags/assets/nixos.svg",
  size: 20
});
...
var time = Variable("", {
  poll: [1e3, function() {
    return format(/* @__PURE__ */ new Date(), "MMM do, HH:mm");
  }]
});
var Time = Widget.EventBox({
  class_name: "date",
  hpack: "center",
  child: Widget.Label({
    label: time.bind()
...
function Volume() {
  ...
  function getIcon() {
    const icon2 = audio.speaker.is_muted ? 0 : [101, 67, 34, 1, 0].find(
      (threshold) => threshold <= audio.speaker.volume * 100
    );
    return `audio-volume-${icons[icon2]}-symbolic`;
  }
